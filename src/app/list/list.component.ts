...
export class ListComponent {
  ...
  private formPedExec = inject(FormBuilder)
  ...
  //Tela de Pedido de Execução
  @ViewChild('ttPedExec') GridPedExec!: PoTableComponent
  @ViewChild('telaPedExec', { static: true }) telaPedExec:  | PoModalComponent  | undefined;
  @ViewChild('telaAlteraPedExec', { static: true }) telaAlteraPedExec:  | PoModalComponent  | undefined;
  ...
  loadTelaPedExec: boolean = false
  labelLoadTelaPedExec:string = ''
  loadExcel: boolean = false
  listaEstabelecimentos: any[] = [];
  alturaGrid:number=window.innerHeight - 295
  alturaGridPedExec:number=window.innerHeight - 480
  codFilial!: string;
  registros!: any[];
  registrosPedExec!: any[];
  ...
  //Form Altera do PedExec
  public formAlteraPedExec = this.formPedExec.group({
    "cPedExec": [0],
    "cPedExecPai": [0],
  })
...
acaoLogin_cancel: PoModalAction = {
  ...
  //Alterar PedExec
  onAlterarPedExec(obj: any | null){

    this.objSolic = [obj.NumPedExec]
    
    this.linhaSelecionada = obj
    this.telaAlteraPedExec?.open();
    
    //setTimeout(() => {
    //  document.getElementById ('iPedExecPai')?.focus()
      //pai.focus()
    //}, 1000); 
    ...
    if (obj !== null) {
      this.formAlteraPedExec.controls.cPedExec.setValue(obj.NumPedExec)
      this.formAlteraPedExec.controls.cPedExecPai.setValue(obj.NumPedExecPai.toString())
    }
  ...
  readonly acaoCancelarLinha: PoModalAction = {
    ...
    action: () => {
      ...
      this.telaPedExec?.close()
    },
  }

  readonly acaoAlterarLinhaPedExec: PoModalAction = {
    label: 'Salvar',
    action: () => {
      this.onSalvarPedExec()},
   
    disabled: !this.formAlteraPedExec.valid,
  };

  readonly acaoCancelarLinhaPedExec: PoModalAction = {
    label: 'Cancelar',
    action: () => {
      this.telaAlteraPedExec?.close()
    },
  }
  ConsultaPedExec(){

    this.telaPedExec?.open()
    this.labelLoadTelaPedExec = "Carregando PedExec"
  
    this.colunasPedExec = this.srvTotvs.ObterColunasPedExec()
  
    this.registrosPedExec = []

    this.loadTelaPedExec = true;
    let params = { CodFilial: this.codFilial };
      this.srvTotvs.PopularPedExec(params).subscribe({
        next: (data: { items: Array<any> }) => { 
                        if (data !== null) {
                          
                          this.registrosPedExec = data.items
                        }
                        this.loadTelaPedExec = false
                        },
        error: (error:any) => { this.Pnotifica.error("Ocorreu um erro:" + error) 
                                this.loadTelaPedExec = false
        },
  ...
  onSalvarPedExec(){
    ...
    //console.log(this.formAlteraPedExec.controls.cPedExecPai.value)
    this.linhaSelecionada.cPedExecPai = this.formAlteraPedExec.controls.cPedExecPai.value

    let registro = {...this.linhaSelecionada, 'NumPedExecPai': Number(this.formAlteraPedExec.controls['cPedExecPai'].value)}
    this.GridPedExec?.updateItem(this.linhaSelecionada, registro)
    this.registrosPedExec = (this.GridPedExec?.items as any[])
    this.telaAlteraPedExec?.close()
    alert (1)
    
    //Chamar o servico
    alert(2)
    this.srvTotvs.AltPedExec(registro).subscribe({
          next: (response: any) => {
            this.loadTela = false
            this.srvNotification.success('Execução do empréstimo realizada com sucesso ! Processo RPW: ' + response)
           
          },
          error: (e) => {
            this.srvNotification.success('erro: ' + e)
            this.loadTela = false;
          },
    });
  }
  ...
  modalActions =[
    ...
    {label: 'cancelar', action: () => this.onSalvarPedExec()},
    {label: 'Salvar', action: () => this.onSalvarPedExec()},
    {label: 'Excel', action: () => this.onSalvarPedExec()}

  ]

  public onExportarExcelPedExec() {
    let titulo = "PED.EXEC." //this.tituloTela.split(':')[0]
    let subTitulo = "LISTA DE PEDIDOS DE EXECUÇÃO" //this.tituloTela.split(':')[1]
    this.loadExcel = true

    //let valorForm: any = { valorForm: this.form.value }
    this.srvExcel.exportarParaExcel('PEDIDOS DE EXECUÇÃO: ' + titulo.toUpperCase(),
      subTitulo.toUpperCase(),
      this.colunasPedExec,
      this.registrosPedExec,
      'PedExec_' + this.codFilial,
      'Plan1')
