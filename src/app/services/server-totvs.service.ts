...
export class ServerTotvsService {
  ...
  //Coluna PedExec
  ObterColunasPedExec(): Array<PoTableColumn>{
    return[
      { property: 'CodUsuario',      label: "Usuario"},
      { property: 'NumPedExec',      label: "PedExec"},
      { property: 'NumPedExecPai',   label: "PedExec Pai"},
      { property: 'ServidExec',      label: "Servidor Exec"},
      { property: 'DatExec',         label: "Data Exec"},
      { property: 'opcao',           label: "", type:'cellTemplate'}
  ...
  public LoginAdmin(params?: any) {
    return this.http.post(`${this._url}/LoginAdmin`, params, { headers: headersTotvs }).pipe(take(1));
  }

  //Lista SaldoTerceiros via RPW
  public ObterListaSaldoTerc(params?: any) {
    return this.http.get(`${this._url}/PrListaSaldoTerc`, {params, headers: headersTotvs}).pipe(take(1));
  }
  ...
  //Grid de Pedido de Execução  
  public PopularPedExec(params?:any) {
    return this.http.get<any>(`${this._url}/PrPedExec`, {params:params, headers:headersTotvs}).pipe(take(1));
    //return this.http.get<any>("https://hawebdev.dieboldnixdorf.com.br:8543/api/integracao/aat/v1/esaa002api/PrRecebeRenovMensal", {params: params, headers:headersTotvs}).pipe(take(1));
  }

  public AltPedExec(params?:any) {
    return this.http.post(`${this._url}/PrAltPedExec`, params, {headers:headersTotvs}).pipe(take(1))
  }
